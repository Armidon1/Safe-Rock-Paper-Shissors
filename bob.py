...
def sendWhoIAm(socket):
    ...
    # use Bob's private key for signing
    bob_private_key = load_private_key("bob_key.pem")
    timestamp = time.time()
...
def game(message, conn):
    ...
    send_json_encrypted(message, conn, "bob",session_key)
    return True
